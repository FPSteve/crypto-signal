...
export function MarketMetricGrid({ metrics }: MarketMetricGridProps) {
  return (
    <section className="border-b border-[var(--rule)] px-5 py-5 sm:px-8">
      <div className="mx-auto grid max-w-7xl grid-cols-1 divide-y divide-[var(--rule)] border border-[var(--rule)] bg-[var(--bg-card)] sm:grid-cols-2 sm:divide-x sm:divide-y-0 lg:grid-cols-6">
        {metrics.map((metric) => (
